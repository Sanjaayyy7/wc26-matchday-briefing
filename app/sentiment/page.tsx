        ...
        >
          <DataPlane>
            <p className="text-caption max-w-2xl mb-4">
              Model: <span className="text-[var(--ink)] font-mono text-xs">{modelName}</span>.
          ...
          <CanvasSection eyebrow="No data" title="No scored fixtures yet.">
            <DataPlane>
              <p className="text-caption">
                Run <code className="font-mono text-xs">npm run sentiment:score -- &lt;slug&gt;</code> to generate sentiment data.
              </p>
            </DataPlane>
          </CanvasSection>
        )}
              ...
              <DataPlane>
                <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                  <div>
                    ...
                    <p className="text-display tabular text-3xl mt-2">{s.counts.total}</p>
                  </div>
                  <div>
                    <p className="text-label">Positive</p>
                    <p className="text-display tabular text-3xl mt-2 text-[var(--up)]">{posP}%</p>
                  </div>
                  <div>
                    <p className="text-label">Negative</p>
                    <p className="text-display tabular text-3xl mt-2 text-[var(--down)]">{negP}%</p>
                  </div>
                  <div>
                    <p className="text-label">Biggest swing</p>
                    <p className={`text-display tabular text-3xl mt-2 ${s.swing && s.swing.delta < 0 ? "text-[var(--down)]" : "text-[var(--up)]"}`}>
