...
export function ChampionshipProjection({ projections }: { projections: ChampionProjection[] }) {
  ...
  return (
    <div className="p-4">
      <div className="text-label font-semibold text-[var(--ink)] mb-3">
        Championship projection
        <span className="ml-1 text-fine font-normal text-[var(--ink-faint)]">· 10k simulations</span>
      </div>
      <div>
        {projections.map((p) => (
          <div key={p.team} className="flex items-center gap-1.5 py-1 border-b border-[var(--hairline)] last:border-0">
            <span className="text-fine text-[var(--ink-faint)] w-2.5 flex-shrink-0">{p.rank}</span>
            <span className={`text-slight flex-1 truncate ${p.rank === 1 ? "font-medium text-[var(--ink)]" : "text-[var(--ink-muted)]"}`}>
              {p.team}
            </span>
